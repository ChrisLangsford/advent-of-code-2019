...
console.log(`Day 2, Part 1: ${intCode(input, 12, 2)}`);
console.log(`Day 2, Part 2: ${day2pt2(input)}`);
...
function intCode(initialState, noun, verb) {
    ...
    for (let instructionPointer = 0; instructionPointer < programState.length; instructionPointer += 4) {
        ...
        switch (operation) {
            case 1:
                programState[programState[instructionPointer + 3]] = add(findInList(programState, programState[instructionPointer + 1]), findInList(programState, programState[instructionPointer + 2]));
                break;
            case 2:
                programState[programState[instructionPointer + 3]] = multiply(findInList(programState, programState[instructionPointer + 1]), findInList(programState, programState[instructionPointer + 2]));
                break;
            case 99:
...
function day2pt2(initialState) {
    ...
    console.log("Starting day 2 pt 2");
    let terminate = false;
    ...
    for (let noun = 0; !terminate && noun <= 99; noun++) {
        for (let verb = 0; !terminate && verb <= 99; verb++) {
            ...
            if (result == 19690720) {
                ...
                console.log(`Program output: ${100 * noun + verb}`);
            }
        }
    }
}
