...
function traceWire(wireInstruction, grid, wireNumber) {
    ...
    wireInstruction.split(",").forEach((instructionString) => {
        ...
        for (let i = 0; i < instruction.range; i++) {
            ...
            if (grid[currentY][currentX] === 1 || grid[currentY][currentX] === 2) {
                grid[currentY][currentX] = 'X';
            }
            grid[currentY][currentX] = wireNumber;
            console.log(JSON.stringify(grid));
        }
    });
}

function buildInstruction(wireInstruction) {
    return {
        direction: wireInstruction.slice(0, 1),
        range: parseInt(wireInstruction.slice(1, wireInstruction.length))
    }
